...
\ifx\islight\undefined
    ...
    \renewcommand{\myfootertext}{Appunti redatti per gli studenti del DISI. \\ Eventuali errori possono essere segnalati su \href{https://github.com/mirconegri/University/tree/main/2nd_semester/fondamenti/schemi/}{GitHub}.}
    \pagecolor{black}
    \color{white}
\else
    ...
    \renewcommand{\myfootertext}{Versione Light Mode. Disponibile anche la \href{https://github.com/mirconegri/University/tree/main/2nd_semester/fondamenti/schemi/}{versione Dark Mode}.}    \pagecolor{white}
    \color{black}
\fi
...
\begin{titlepage}
    ...
    \href{https://github.com/mirconegri/University}{%
        \begin{tabular}{@{}c@{}}
            ...
            {\Huge\bfseries Appunti di Fondamenti matematici}\\ [0.2cm] {\Huge\bfseries per l'informatica}  \\[0.4cm]
...
\subsection{Struttura Algebrica e Operazioni in $\mathbb{Z}_n$}
L'insieme delle classi di resto $\mathbb{Z}_n$ possiede una struttura di \textbf{anello commutativo con unità}. Le operazioni di somma e prodotto sono ereditate da $\mathbb{Z}$ e sono \textit{ben definite}:
\begin{itemize}
...
Per ogni $[a]_n, [b]_n, [c]_n \in \mathbb{Z}_n$, valgono:
\begin{itemize}
    \item \textbf{Associatività e Commutatività} sia per la somma che per il prodotto.
    \item \textbf{Elementi Neutri:} $[0]_n$ per la somma, $[1]_n$ per il prodotto.
    \item \textbf{Esistenza dell'Opposto:} Per ogni $[a]_n$, esiste $[-a]_n = [n-a]_n$ tale che la loro somma sia $[0]_n$.
    \item \textbf{Proprietà Distributiva} del prodotto rispetto alla somma.
\end{itemize}

\subsection{Elementi Invertibili e Calcolo dell'Inverso}
Diciamo che un elemento $[a]_n \in \mathbb{Z}_n$ è \textbf{invertibile} se esiste una classe $[x]_n \in \mathbb{Z}_n$ tale che:
\[ [a]_n \odot [x]_n = [1]_n \]
In tal caso, $[x]_n$ è l'\textbf{inverso moltiplicativo} di $[a]_n$ e si indica con $[a]_n^{-1}$.

\subsubsection*{Condizione di Invertibilità}
Il criterio fondamentale per l'esistenza dell'inverso è basato sulla coprimarità:
\[ [a]_n \text{ è invertibile in } \mathbb{Z}_n \iff \text{MCD}(a, n) = 1 \]
\emph{Conseguenza:} $\mathbb{Z}_n$ è un \textbf{campo} (ogni elemento non nullo è invertibile) se e solo se $n$ è un \textbf{numero primo}. Se $n$ è composto, esistono dei \textbf{divisori dello zero}.

\subsubsection*{Metodo di Calcolo (Algoritmo di Euclide Esteso)}
Per calcolare l'inverso di $[a]_n$ modulo $n$:
\begin{enumerate}
    \item Si applica l'algoritmo di Euclide per trovare $1 = \text{MCD}(a, n)$.
    \item Si risale l'algoritmo (Identità di Bézout) per scrivere: $1 = s \cdot a + t \cdot n$.
    \item L'inverso cercato è $[a]_n^{-1} = [s]_n$.
\end{enumerate}

\paragraph{Esercizio Tipo: Calcolo dell'inverso di $[8]_{35}$}
\begin{enumerate}
    \item \textbf{Verifica:} $\text{MCD}(8, 35) = 1$. L'inverso esiste.
    \item \textbf{Euclide:} 
    $35 = 4 \cdot 8 + 3 \implies 3 = 35 - 4 \cdot 8$ \\
    $8 = 2 \cdot 3 + 2 \implies 2 = 8 - 2 \cdot 3$ \\
    $3 = 1 \cdot 2 + 1 \implies 1 = 3 - 1 \cdot 2$
    \item \textbf{Bézout (Risalita):} 
    $1 = 3 - 1 \cdot (8 - 2 \cdot 3) = 3 \cdot 3 - 8$ \\
    $1 = 3 \cdot (35 - 4 \cdot 8) - 8 = 3 \cdot 35 - 13 \cdot 8$
    \item \textbf{Risultato:} $1 \equiv -13 \cdot 8 \pmod{35}$. L'inverso è $[-13]_{35}$. \\
    In forma positiva: $-13 + 35 = 22$. Quindi $[8]_{35}^{-1} = [22]_{35}$.
\end{enumerate}
...
\subsection{Sistemi di Congruenze e Teorema Cinese dei Resti}
...
Un sistema ammette soluzioni (è \textbf{compatibile}) se e solo se:
\[ a \equiv b \pmod{\text{MCD}(n, m)} \iff \text{MCD}(n, m) \mid (a - b) \]
Se i due moduli sono coprimi ($\text{MCD}(n, m) = 1$), la condizione è sempre verificata e il sistema ammette sempre soluzione.

\subsubsection*{Risoluzione e Insieme delle Soluzioni}
Una volta verificata la compatibilità, la soluzione si ricava tramite l'Identità di Bézout: $\text{MCD}(n, m) = s \cdot n + t \cdot m$.
Costruita una soluzione particolare $x_0$, il teorema afferma che l'insieme di tutte le soluzioni $S$ è unico modulo $\text{m.c.m.}(n, m)$:
\[ S = \{ x_0 + k \cdot \text{m.c.m.}(n, m) \mid k \in \mathbb{Z} \} \]
