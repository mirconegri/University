...
\title{Appunti di Calcolatori - Introduzione}
\author{Trascrizione dalle fonti del corso}
\date{}
...
\begin{itemize}
    \item \textbf{Corso}: CALCOLATORI - Aritmetica dei Calcolatori. 
    \item \textbf{Docente}: Giovanni Iacca (basato su materiale del Prof. Luca Abeni). 
    \item \textbf{Istituto}: Università degli Studi di Trento, Dipartimento di Ingegneria e Scienza dell'Informazione. 
\end{itemize}
...
\begin{itemize}
    ...
    \item Lo standard IEEE 754 utilizza una codifica normalizzata in C per esponenti traslati $E' = E + 2^{e-1} - 1$. 
    \item \textbf{Numeri Normalizzati}: Si usano quando $E' > 0$. La mantissa viene rappresentata con la forma $1.M$ e l'esponente vale $E = E' - (2^{e-1} - 1)$. 
    \item \textbf{Numeri Denormalizzati}: Si usano quando $E' = 0$, per gestire l'underflow vicini allo zero. La mantissa assume forma $0.M$. 
    \item \textbf{Casi Particolari}:
    \begin{itemize}
        \item \textbf{Zero}: Esponente 0 e mantissa 0. 
        \item \textbf{Infinito}: Esponente tutto a 1 (es. 255 per float 32-bit) e mantissa 0. 
        \item \textbf{NaN (Not a Number)}: Esponente tutto a 1 e mantissa diversa da zero. 
    \end{itemize}
\end{itemize}
...
L'obiettivo principale di questo corso è mostrare come funziona, e come si progetta, un calcolatore. Per fare ciò, è necessaria una piccola digressione su come si progettano i circuiti logici.
...
Le reti logiche sono circuiti che trasformano alcuni valori logici in ingresso in altri valori logici in uscita. Si dividono in due tipi principali:
\begin{itemize}
    \item \textbf{Reti Combinatorie}: Rappresentano una relazione funzionale tra ingresso e uscita. Non hanno memoria, poiché l'uscita dipende solo dal valore attuale dell'ingresso. Possono essere specificate tramite una \textit{tabella di verità}.
    \item \textbf{Reti Sequenziali}: L'uscita dipende dalla storia degli ingressi passati e non solo dal valore attuale. Esse possiedono una memoria, definita anche come \textit{stato} della rete.
\end{itemize}
...
\subsection{Linguaggio Assembler: Istruzioni Logiche, Scorrimento e Salti}
\begin{table}[htbp]
\centering
\resizebox{\textwidth}{!}{%
