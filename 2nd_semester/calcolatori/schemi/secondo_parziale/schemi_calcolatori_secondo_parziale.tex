...
\ifx\islight\undefined
    ...
    \renewcommand{\myfootertext}{Appunti redatti per gli studenti del DISI. \\ Eventuali errori possono essere segnalati su \href{https://github.com/mirconegri/University/tree/main/2nd_semester/calcolatori/schemi/}{GitHub}.}
    \pagecolor{black}
    \color{white}
\else
    ...
    \renewcommand{\myfootertext}{Versione Light Mode. Disponibile anche la \href{https://github.com/mirconegri/University/tree/main/2nd_semester/calcolatori/schemi/}{versione Dark Mode}.}    \pagecolor{white}
    \color{black}
\fi
...
\begin{titlepage}
    ...
    \href{https://github.com/mirconegri/University}{%
        \begin{tabular}{@{}c@{}}
            ...
            {\Huge\bfseries Appunti di Calcolatori} \\[0.3cm]
            {\Large\itshape Secondo parziale} \\[0.4cm]
...
\section{Esempi di Programmi Assembly: RISC-V, Intel x86-64 e ARM}

In questa sezione analizzeremo la traduzione di vari costrutti di programmazione ad alto livello in linguaggio Assembly, confrontando l'architettura RISC-V, l'architettura Intel x86-64 e quella ARM. Questo confronto ci permetterà di apprezzare come le peculiarità di ciascun processore (come l'esecuzione condizionale in ARM o l'indirizzamento complesso in Intel) semplifichino e differenzino notevolmente la stesura del codice.

\subsection{Richiamo: Registri e Convenzioni}
Prima di procedere con gli esempi, ricordiamo brevemente l'uso dei registri nelle tre architetture:
\begin{itemize}
    ...
    \item \textbf{Intel x86-64:} Dispone di 16 registri general-purpose a 64 bit. I parametri passano in \texttt{\%rdi, \%rsi, \%rdx, \%rcx, \%r8, \%r9}, mentre il valore di ritorno va in \texttt{\%rax}. Lo stack pointer è \texttt{\%rsp} e il base pointer è \texttt{\%rbp}. I registri \texttt{\%rbx, \%rbp, \%r12-\%r15} sono preservati dal chiamato (\textit{callee-saved}).
...
Supponiamo \texttt{g, h, i, j} passati nei registri \texttt{\%edi, \%esi, \%edx, \%ecx}. Il risultato \texttt{f} andrà in \texttt{\%eax}.
\begin{verbatim}
leal (%rdi, %rsi), %eax    // %eax = g + h
addl %ecx, %edx            // %edx = i + j
subl %edx, %eax            // %eax = %eax - %edx (f = ...)
\end{verbatim}

\subsubsection{Traduzione ARM}
La logica è identica a RISC-V, differiscono solo i nomi dei registri. L'uso della \texttt{s} finale (es. \texttt{adds}) serve per far aggiornare i flag di stato alla ALU, anche se per questa specifica espressione non è strettamente necessario.
\begin{verbatim}
...
\subsubsection{Traduzione Intel x86-64}
Assumiamo \texttt{h} in \texttt{\%esi} e l'indirizzo base dell'array \texttt{a} in \texttt{\%rdi}. Poiché gli offset (32 e 48) sono costanti statiche, il compilatore le pre-calcola e le usa come \textit{displacement} diretto.
\begin{verbatim}
movl 32(%rdi), %eax        // %eax = a[8]
addl %esi, %eax            // %eax = h + a[8]
movl %eax, 48(%rdi)        // a[12] = %eax
\end{verbatim}

\subsubsection{Traduzione ARM}
L'indirizzamento è molto simile a RISC-V (pre-indexed senza writeback). Assumiamo \texttt{h} in \texttt{r0} e base \texttt{a} in \texttt{r1}.
\begin{verbatim}
ldr r3, [r1, #32]   // r3 = a[8]
...
\subsubsection{Traduzione Intel x86-64 (Ottimizzazione con Conditional Move)}
Assumiamo \texttt{i, j, g, h} in \texttt{\%edi, \%esi, \%edx, \%ecx}. Intel x86-64 può evitare i salti svuota-pipeline calcolando entrambi i rami speculativamente e usando un \texttt{cmov} (Conditional Move).
\begin{verbatim}
leal (%rdx, %rcx), %eax    // %eax = g + h (Calcola sempre il Ramo IF)
movl %edx, %r8d            
subl %ecx, %r8d            // %r8d = g - h (Calcola sempre il Ramo ELSE)
cmpl %esi, %edi            // Confronta i e j (imposta i flag)
cmovne %r8d, %eax          // Se i != j (Not Equal), sovrascrivi %eax con %r8d
\end{verbatim}
...
\begin{itemize}
    ...
    \item \textbf{Intel x86-64:} Usa \texttt{cmpl} seguito da \texttt{cmovge} (Move if Greater or Equal).
...
\subsubsection{Traduzione Intel x86-64}
Sfruttiamo la potente modalità di indirizzamento AGU \texttt{(base, indice, scala)} in hardware, che ci risparmia il calcolo manuale dello shift per il contatore. Assumiamo \texttt{a} in \texttt{\%rdi}, \texttt{k} in \texttt{\%edx} e il contatore \texttt{i} in \texttt{\%rax}.
\begin{verbatim}
    movq $0, %rax              // i = 0
L1: cmpl %edx, (%rdi, %rax, 4) // k == a[i]? (Risolve addr = rdi + rax * 4)
    jne  L2                    // Se diversi, esci dal ciclo
    addq $1, %rax              // i += 1
    jmp  L1                    // Torna all'inizio del loop
L2:
\end{verbatim}
...
\subsubsection{Traduzione Intel x86-64}
L'istruzione \texttt{call} in Intel salva automaticamente l'indirizzo di ritorno sullo stack. Il parametro \texttt{x} si trova già in \texttt{\%edi}, che è lo stesso registro richiesto dalla convenzione di chiamata per passarlo in avanti a \texttt{inc}. Serve solo riallineare lo stack pointer.
\begin{verbatim}
f:  subq $8, %rsp              // Allinea lo stack pointer (a 16 byte boundary)
    call inc                   // Chiama inc(x), risultato in %eax
    subl $4, %eax              // Sottrae 4 al valore di ritorno
    addq $8, %rsp              // Ripristina lo stack pointer
    ret
\end{verbatim}
...
f:  stmfd sp!, {r4, lr} // Push di r4 e del Link Register. sp si aggiorna (!)
    ...
    sub   r0, r0, #4    // a0 = a0 - 4
    ldmfd sp!, {r4, pc} // Pop: ripristina r4 e mette l'indirizzo salvato in PC. 
                        // (RITORNO AUTOMATICO)
\end{verbatim}
...
\subsubsection{Traduzione Intel x86-64}
Sfruttiamo l'indirizzamento complesso sui singoli byte. Manteniamo il contatore \texttt{i} in \texttt{\%rcx}, l'array destinazione \texttt{d} in \texttt{\%rdi} e la sorgente \texttt{s} in \texttt{\%rsi}.
\begin{verbatim}
    movq $0, %rcx              // i = 0
L1: movb (%rsi, %rcx, 1), %al  // %al = s[i] (carica 1 byte)
    movb %al, (%rdi, %rcx, 1)  // d[i] = %al (salva 1 byte)
    testb %al, %al             // Controlla se %al è 0 ('\0')
    je   Fine                  // Se è nullo, esci dal ciclo
    addq $1, %rcx              // i += 1
    jmp  L1                    // cicla
Fine:
\end{verbatim}
...
% --- NUOVE DOMANDE AGGIUNTE ---
    ...
    \item \textbf{Domanda:} Si consideri una CPU le cui fasi impiegano: 150ps per il fetch, 350ps per la decodifica, 500ps per la ALU, 400ps per la memoria e 200ps per il write-back. Il massimo incremento di prestazioni atteso con la pipeline è:
    \begin{enumerate}[label=(\Alph*)]
        \item di 3 volte
        \item di 2.8 volte
        \item di 4 volte
    ...
    \end{enumerate}
    \textbf{Risposta corretta: (D)} \\
    \textbf{Perché è corretta:} Calcoliamo il tempo di esecuzione sequenziale di una singola istruzione sommando i tempi delle fasi: $150 + 350 + 500 + 400 + 200 = 1600$ ps. In un'architettura a pipeline, il tempo del ciclo di clock globale è vincolato dallo stadio più lento (il collo di bottiglia), che in questo caso è la ALU con $500$ ps. Lo speedup (incremento di prestazioni) si calcola dividendo il tempo totale sequenziale per il tempo di clock della pipeline: $1600 / 500 = 3.2$. L'incremento atteso è di 3.2 volte. \\
    \textbf{Perché scartare le altre:} Le opzioni (A), (B) e (C) sono il risultato di calcoli errati. Ad esempio, (A) deriverebbe se si facesse una stima approssimativa o (B) se il clock fosse calcolato su un'altra fase scorretta.

\newpage

    \item \textbf{Domanda:} Si consideri una cache set associative a 8 vie grande 32KB, con blocchi di 64 byte. In quali blocchi può essere mappato l'indirizzo \texttt{0x205C0}?
    \begin{enumerate}[label=(\Alph*)]
        \item Nei blocchi dal numero 23 al numero 30
        \item Nei blocchi dal numero 184 al numero 191
        \item Nel blocco numero 184
        \item Nei blocchi dal numero 92 al numero 99
        \item Nessuna delle altre risposte
    \end{enumerate}
    \textbf{Risposta corretta: (B)} \\
    \textbf{Perché è corretta:} Calcoliamo i parametri della cache. Dimensione totale $= 32\text{ KB} = 32768$ byte. Numero totale di blocchi in cache $= 32768 / 64 = 512$ blocchi. Essendo associativa a 8 vie, il numero di Set è $512 / 8 = 64$ Set.
    Convertiamo l'indirizzo in decimale: $\texttt{0x205C0} = 2 \cdot 16^4 + 5 \cdot 16^2 + 12 \cdot 16 = 131072 + 1280 + 192 = 132544$.
    L'indirizzo del blocco in memoria è $\lfloor 132544 / 64 \rfloor = 2071$.
    Troviamo il Set calcolando il modulo: $2071 \bmod 64 = 23$.
    Il dato finisce nel Set logico 23. Poiché ogni Set è formato da 8 vie (8 blocchi fisici contigui), il Set 23 inizia dal blocco fisico $23 \times 8 = 184$ e finisce al blocco $184 + 7 = 191$. \\
    \textbf{Perché scartare le altre:} La (A) indica il Set logico, non i blocchi fisici. La (C) indica un solo blocco ignorando l'associatività a 8 vie. La (D) moltiplicherebbe per 4 invece che per 8.

    \item \textbf{Domanda:} Dato un sistema con indirizzamento a 32 bit e una memoria cache la cui dimensione del blocco (cache line) è di 64 byte, qual è il numero del blocco di memoria in decimale corrispondente all'indirizzo \texttt{0x004A1F2C}?
    \begin{enumerate}[label=(\Alph*)]
        \item 75900
        \item 48576
        \item 18975
    ...
    \end{enumerate}
    \textbf{Risposta corretta: (A)} \\
    \textbf{Perché è corretta:} Per trovare a quale "blocco di memoria" corrisponde un indirizzo, bisogna semplicemente ignorare i bit di Offset (ovvero effettuare una divisione intera per la grandezza del blocco). Convertiamo l'indirizzo esadecimale in decimale: $\texttt{0x004A1F2C} = 4 \cdot 16^5 + 10 \cdot 16^4 + 1 \cdot 16^3 + 15 \cdot 16^2 + 2 \cdot 16 + 12 = 4194304 + 655360 + 4096 + 3840 + 32 + 12 = 4857644$.
    Dividiamo questo indirizzo per la dimensione del blocco (64 byte): $\lfloor 4857644 / 64 \rfloor = 75900$. (L'offset all'interno del blocco è il resto, cioè 44). \\
    \textbf{Perché scartare le altre:} Le altre risposte presentano errori nei calcoli di conversione o omettono la divisione finale per determinare l'indice del blocco.

\newpage

    \item \textbf{Domanda:} Si consideri una cache direct mapped grande 16KB con blocchi da 64 byte. In quale blocco di cache andrà la parola all'indirizzo \texttt{0x81C0}?
    \begin{enumerate}[label=(\Alph*)]
        \item Nel blocco 192
        \item Nel blocco 7
        \item Nel blocco 64
    ...
    \end{enumerate}
    \textbf{Risposta corretta: (B)} \\
    \textbf{Perché è corretta:} Troviamo i blocchi totali della cache: $16\text{ KB} = 16384$ byte. Numero di blocchi $= 16384 / 64 = 256$ blocchi totali.
    Convertiamo l'indirizzo in decimale: $\texttt{0x81C0} = 8 \cdot 16^3 + 1 \cdot 16^2 + 12 \cdot 16 = 32768 + 256 + 192 = 33216$.
    Calcoliamo l'indice del blocco di memoria dividendo per 64 (scartando l'offset): $\lfloor 33216 / 64 \rfloor = 519$.
    Calcoliamo in che blocco di cache verrà mappato tramite l'operatore modulo: $519 \bmod 256 = 7$. \\
    \textbf{Perché scartare le altre:} Le opzioni errate derivano tipicamente dal dimenticare la divisione per l'offset (facendo il modulo direttamente sull'indirizzo anziché sull'indice del blocco) o da un'errata conversione di base.

    \item \textbf{Domanda:} Una CPU presenta un CPI ideale di 5. La cache istruzioni ha un miss rate del 3\% e la cache dati del 6\%. Una cache miss richiede 80 cicli per essere risolta. Se il 25\% delle istruzioni accede alla memoria dati, calcolare il CPI reale.
    \begin{enumerate}[label=(\Alph*)]
        \item 8.6
        \item 7.4
        \item 12.2
    ...
    \end{enumerate}
    \textbf{Risposta corretta: (A)} \\
    \textbf{Perché è corretta:} Il CPI reale si ottiene sommando al CPI ideale (5) le penalità per i miss sulle istruzioni e sui dati.
    1. \textbf{Penalità Istruzioni}: Tutte le istruzioni (100\%) vengono lette in fase di Fetch. Miss Rate = $0.03$. Penalità = $1.0 \times 0.03 \times 80 = 2.4$ cicli per istruzione.
    2. \textbf{Penalità Dati}: Solo il 25\% delle istruzioni esegue un accesso alla memoria dati. Miss Rate = $0.06$. Penalità = $0.25 \times 0.06 \times 80 = 1.2$ cicli per istruzione.
    $\text{CPI Reale} = \text{CPI Ideale} + \text{Penalità Istruzioni} + \text{Penalità Dati} = 5 + 2.4 + 1.2 = 8.6$. \\
    \textbf{Perché scartare le altre:} (C) calcola assumendo che il 100\% delle istruzioni acceda anche alla cache dati ($5 + 2.4 + 4.8 = 12.2$). Le altre opzioni derivano da mancate moltiplicazioni per 80 o da errori nei fattori.

\newpage

    % --- ULTIME DOMANDE AGGIUNTE ---

    \item \textbf{Domanda:} Si consideri una CPU con una pipeline idealmente bilanciata, in cui ciascuna delle 5 fasi di esecuzione impiega esattamente 300ps. Qual è il massimo incremento di prestazioni che ci si aspetta usando la pipeline?
    \begin{enumerate}[label=(\Alph*)]
        \item di 4 volte
        \item di 3 volte
        \item di 5 volte
    ...
    \end{enumerate}
    \textbf{Risposta corretta: (C)} \\
    \textbf{Perché è corretta:} In una pipeline *idealmente bilanciata*, tutti gli stadi hanno esattamente la stessa durata. Il tempo di esecuzione sequenziale di una singola istruzione è la somma dei 5 stadi: $300 \times 5 = 1500$ ps. Il tempo di ciclo della pipeline è dettato dallo stadio più lento, ma essendo tutti uguali, il clock sarà esattamente di $300$ ps. Lo speedup (incremento massimo) si ottiene facendo il rapporto tra il tempo senza pipeline e il tempo con pipeline: $1500 / 300 = 5$. In generale, per una pipeline perfettamente bilanciata a $N$ stadi, lo speedup teorico massimo è esattamente $N$. \\
    \textbf{Perché scartare le altre:} Le altre opzioni (4, 3, 6) non corrispondono al numero di stadi della pipeline indicata, fallendo l'applicazione della formula dello speedup ideale.

    \item \textbf{Domanda:} Si consideri una cache set associative a 2 vie grande 4KB, con blocchi da 32 byte. Dove sarà mappata la parola all'indirizzo \texttt{0x41E8}?
    \begin{enumerate}[label=(\Alph*)]
        \item Nel blocco numero 30 o nel blocco numero 31
        \item Nel blocco 15 o 16
        \item Nel blocco 60 o 61
        \item Nel blocco 30 e basta
        \item Nessuna delle altre risposte
    \end{enumerate}
    \textbf{Risposta corretta: (A)} \\
    \textbf{Perché è corretta:} Procediamo per passi. 1) Troviamo il numero totale di blocchi fisici: Dimensione $4\text{ KB} = 4096$ byte. Blocchi totali = $4096 / 32 = 128$ blocchi. 2) Essendo associativa a 2 vie, raggruppiamo i blocchi in \textit{Set}: $128 / 2 = 64$ Set totali. 3) Convertiamo l'indirizzo esadecimale \texttt{0x41E8} in decimale: $4 \cdot 16^3 + 1 \cdot 16^2 + 14 \cdot 16 + 8 = 16384 + 256 + 224 + 8 = 16872_{10}$. 4) Calcoliamo l'indice del blocco di memoria originario dividendo per la dimensione del blocco (32): $\lfloor 16872 / 32 \rfloor = 527$. 5) Troviamo il Set di destinazione con l'operatore modulo: $527 \bmod 64 = 15$. Il dato finirà nel \textbf{Set 15}. 6) Poiché ogni Set è composto da 2 vie contigue, il Set 15 ingloba fisicamente i blocchi della cache numero $15 \times 2 = \textbf{30}$ e $15 \times 2 + 1 = \textbf{31}$. \\
    \textbf{Perché scartare le altre:} La (B) indica il numero del Set logico (15) e il successivo (16), non i blocchi fisici che lo compongono. La (C) userebbe un moltiplicatore errato. La (D) dimentica che la cache è a 2 vie, affermando che il blocco possibile è solo uno.

\newpage

    \item \textbf{Domanda:} Una cache fully associative grande 64KB ha blocchi da 256 byte. Il blocco di memoria corrispondente all'indirizzo \texttt{0xFFE0} verrà scritto:
    \begin{enumerate}[label=(\Alph*)]
        \item In un qualsiasi blocco della cache determinato dalla politica di rimpiazzo
        \item Nel blocco 255
        \item Nel blocco 64
        \item Nei primi 256 blocchi
        \item Nessuna delle altre risposte
    \end{enumerate}
    \textbf{Risposta corretta: (A)} \\
    \textbf{Perché è corretta:} La domanda è teorica e a trabocchetto. Per definizione, in una memoria cache \textbf{completamente associativa (Fully Associative)} non esiste alcun indice vincolante. L'intera cache agisce come un unico, gigantesco Set. Di conseguenza, un blocco di memoria può essere salvato in \textbf{qualsiasi blocco fisico libero} della cache, indipendentemente dal suo indirizzo. Nel caso la cache fosse piena, il blocco da sovrascrivere viene scelto unicamente dall'algoritmo di rimpiazzo (es. LRU o FIFO). \\
    \textbf{Perché scartare le altre:} Fornire un numero di blocco specifico (B, C) o un range limitato (D) è una proprietà delle cache Direct Mapped o Set-Associative, il che contraddice la natura stessa della Fully Associative. I dati numerici forniti servivano solo a confondere.

    \item \textbf{Domanda:} Si consideri una cache direct mapped grande 2KB con blocchi da 16 byte. Trovare il blocco in cui viene mappato l'indirizzo \texttt{0x1034}.
    \begin{enumerate}[label=(\Alph*)]
        \item Nel blocco 52
        \item Nel blocco 16
        \item Nel blocco 3
    ...
    \end{enumerate}
    \textbf{Risposta corretta: (C)} \\
    \textbf{Perché è corretta:} 1) Calcoliamo il numero totale di blocchi della cache: Dimensione $2\text{ KB} = 2048$ byte. Numero di blocchi $= 2048 / 16 = 128$ blocchi totali. 2) Convertiamo l'indirizzo esadecimale \texttt{0x1034} in decimale: $1 \cdot 16^3 + 0 + 3 \cdot 16 + 4 = 4096 + 48 + 4 = 4148_{10}$. 3) Ricaviamo l'indice del blocco in memoria RAM ignorando l'offset: $\lfloor 4148 / 16 \rfloor = 259$. 4) Calcoliamo il blocco di destinazione nella cache con il modulo: $259 \bmod 128 = 3$. L'indirizzo atterrerà quindi nel \textbf{blocco numero 3}. \\
    \textbf{Perché scartare le altre:} Le opzioni errate derivano da calcoli aritmetici olografici sbagliati (es. mancata divisione per la grandezza del blocco prima di applicare il modulo, o fallita conversione da Hex a Decimale).

\newpage

    \item \textbf{Domanda:} Calcolare il CPI reale di una CPU sapendo che: il CPI ideale è 3, la percentuale di cache miss sulle istruzioni è dell'1.5\%, la percentuale di miss sui dati è del 4\%, la miss penalty ammonta a 120 cicli e il 40\% del codice assembly esegue operazioni di load/store (accesso dati in memoria).
    \begin{enumerate}[label=(\Alph*)]
        \item 4.8
        \item 8.72
        \item 5.2
    ...
    \end{enumerate}
    \textbf{Risposta corretta: (D)} \\
    \textbf{Perché è corretta:} Applichiamo la formula dei cicli di stallo persi sommandoli al CPI base (3).
    1. \textbf{Stallo Istruzioni}: Tutte le istruzioni vengono lette in fase di Fetch ($100\%$ frequenza). Cicli persi: $1.0 \times 0.015 \times 120 = 1.8$ cicli/istruzione.
    2. \textbf{Stallo Dati}: Solo il $40\%$ delle istruzioni fa ricorso alla cache dati. Cicli persi: $0.40 \times 0.04 \times 120 = 1.92$ cicli/istruzione.
    Calcoliamo il totale: $\text{CPI Reale} = \text{CPI Ideale} + \text{Stalli Istr.} + \text{Stalli Dati} = 3 + 1.8 + 1.92 = \textbf{6.72}$. \\
    \textbf{Perché scartare le altre:} La (B) calcolerebbe $8.72$ ignorando la frequenza del 40\% sulle load/store, trattandole come se fossero il 100\% ($3 + 1.8 + 4.8 = 9.6$ circa oppure derivata da errori di somma). La (A) e la (C) si limitano a calcoli parziali omesse addizioni del CPI ideale base.

    % --- NUOVE DOMANDE ESTRATTE DAL FILE 260527_Soluzioni.pdf ---

    \item \textbf{Domanda:} Quale delle seguenti proposte descrive meglio il contenuto di un file oggetto?
    \begin{enumerate}[label=(\Alph*)]
        \item Il file oggetto viene prodotto dal linker e contiene il codice eseguibile pronto per essere eseguito.
        \item Il file oggetto contiene oltre al codice tradotto, anche informazioni sui simboli e le funzioni utilizzate.
        \item Il file oggetto è un file di testo che contiene il codice sorgente del programma.
        \item Nel file oggetto tutti gli indirizzi di memoria sono risolti e pronti per essere eseguiti.
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (B)} \\
    \textbf{Perché è corretta:} Il file oggetto viene generato dall'assemblatore e contiene il codice macchina tradotto unito a tabelle fondamentali, come la Symbol Table (informazioni sui simboli e le funzioni utilizzate o esportate) e la Tabella di Rilocazione, essenziali per il successivo processo di linking. \\
    \textbf{Perché scartare le altre:} La prima opzione è errata in quanto il file oggetto è prodotto dal compilatore/assemblatore e non dal linker, e rappresenta un codice intermedio non ancora eseguibile. La terza opzione è falsa perché non è un file di testo col codice sorgente. La quarta opzione è errata poiché gli indirizzi di memoria (specialmente i riferimenti esterni) non sono tutti risolti, ma verranno rilocati e risolti solo successivamente dal linker.

\newpage

    \item \textbf{Domanda:} Quale delle seguenti affermazioni descrive correttamente gli interrupts?
    \begin{enumerate}[label=(\Alph*)]
        \item Gli interrupts sono segnali gestiti in maniera sincrona dal processore.
        \item Gli interrupts causano sempre (o quasi) l'abort del programma in esecuzione.
        \item Gli interrupts sono causati da una specifica richiesta del sistema.
        \item Gli interrupts possono essere gestiti tra due istruzioni.
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (D)} \\
    \textbf{Perché è corretta:} Gli interrupts possono essere gestiti tra l'esecuzione di due istruzioni. Quando si verifica l'evento, il processore completa l'istruzione corrente, sospende il programma, salva lo stato dei registri e avvia la routine di gestione (ISR). Al termine, riprende l'esecuzione da dove era stata interrotta[cite: 14, 15]. \\
    \textbf{Perché scartare le altre:} Gli interrupts sono asincroni, generati da eventi esterni al programma in esecuzione (come un segnale I/O)[cite: 13, 14]. Non causano l'abort del programma ma lo sospendono momentaneamente. Le richieste specifiche del sistema o anomalie sincrone si definiscono eccezioni (o traps), non interrupts.

    \item \textbf{Domanda:} Sia una memoria cache del tipo direct mapped, questa è grande 16KB con blocchi grandi 32 bytes, in quale blocco di questa cache potremmo trovare il dato con indirizzo \texttt{0x65F3AB}?
    \begin{enumerate}[label=(\Alph*)]
        \item Nel blocco \texttt{0x143} della cache.
        \item Nel blocco \texttt{0x19E} della cache.
        \item Nel blocco \texttt{0x21B2} della cache.
        \item Nel blocco \texttt{0x21C4} della cache.
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (E)} \\
    \textbf{Perché è corretta:} Calcoliamo i campi dell'indirizzo: ogni blocco è di $32$ bytes, quindi l'Offset richiede $\log_2(32) = 5$ bit. La cache ha $16\text{KB} / 32\text{B} = 512$ blocchi, richiedendo $\log_2(512) = 9$ bit per l'Indice del blocco[cite: 20, 21]. L'indirizzo binario di \texttt{0x65F3AB} è \texttt{0110 0101 1111 0011 1010 1011}. Estraendo i 9 bit dell'indice (dal bit 5 al bit 13), otteniamo \texttt{1 1001 1101}, che in esadecimale equivale a \texttt{0x19D}. Dato che \texttt{0x19D} non è tra le opzioni (A, B, C, D), la risposta corretta è "Nessuna delle precedenti". \\
    \textbf{Perché scartare le altre:} Le opzioni A, B, C e D riportano blocchi errati calcolati in modo inesatto (es. includendo offset o operando con basi logaritmiche errate).

\newpage

    \item \textbf{Domanda:} Supponendo un processore che possa eseguire un'istruzione in 3 cicli di clock nel caso migliore, per accedere alla memoria istruzioni impiega 100 cicli di clock ma la frequenza di cache-miss per questa memoria è del 1\%. Inoltre il 40\% delle istruzioni eseguite accede alla memoria dati almeno una volta. Se l'accesso alla memoria dati impiega 200 cicli di clock e la frequenza di cache-miss per questa memoria è del 5\%, quanti cicli di clock in media impiega il processore per eseguire un'istruzione?
    \begin{enumerate}[label=(\Alph*)]
        \item 5 cicli di clock.
        \item 10 cicli di clock.
        \item 7 cicli di clock.
        \item 8 cicli di clock.
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (D)} \\
    \textbf{Perché è corretta:} Si applica la formula per il calcolo del tempo medio (CPI Reale). I cicli per istruzione ideali sono $T_{ist} = 3$. La penalità per le istruzioni è il miss rate (1\%) moltiplicato per il costo del miss (100 cicli) = $0.01 \times 100 = 1$ ciclo. La penalità per i dati si applica solo al 40\% delle istruzioni, con un miss rate del 5\% e costo di 200 cicli: $0.40 \times (0.05 \times 200) = 0.40 \times 10 = 4$ cicli. Sommando: $3 + 1 + 4 = 8$ cicli medi per istruzione. \\
    \textbf{Perché scartare le altre:} Le altre risposte sono figlie di errori nell'applicazione della formula, tipicamente dimenticandosi di calcolare la probabilità di accesso alla memoria dati (40\%) o valutando male i costi.

    \item \textbf{Domanda:} Viene dato un frammento C e una traduzione ARM incompleta. Quali sono le istruzioni da inserire al posto di \texttt{X1} e \texttt{X2} per completare correttamente la traduzione?
\begin{verbatim}
long long int supreamSum (long long int * a, long long int * b, 
                          short n, short m) {
    short i = 0;
    long long int sum = 0;
    while (i < n && i < m) {
...
\end{verbatim}
    \begin{enumerate}[label=(\Alph*)]
        \item \texttt{X1: ldr r2, [r4, \#8]} | \texttt{X2: pop \{r4, r5, r6, pc\}}
        \item \texttt{X1: ldr r2, [r4, \#8]!} | \texttt{X2: pop \{r4, r5, r6, pc\}}
        \item \texttt{X1: ldr r2, [r4, \#4]} | \texttt{X2: pop \{r4, r5, r6, lr\}}
        \item \texttt{X1: ldr r2, [r4, \#4]!} | \texttt{X2: pop \{r4, r5, r6, pc\}}
        \item Nessuna delle precedenti.
    \end{enumerate}
...
\newpage
    ...
    \textbf{Risposta corretta: (B)} \\
    \textbf{Perché è corretta:} Il codice lavora su \texttt{long long int}, i cui elementi occupano 8 byte. Nel ciclo per scorrere l'array \texttt{b}, il compilatore deve aggiornare il puntatore caricando contemporaneamente il valore e spostandosi avanti di 8 byte per l'iterazione successiva. L'istruzione in \texttt{X1} utilizza l'indirizzamento pre-indexed con writeback (indicato col suffisso \texttt{!}): \texttt{ldr r2, [r4, \#8]!}[cite: 59, 60, 61]. Alla fine della funzione (\texttt{X2}), i registri salvati in testa con \texttt{push} devono essere estratti e l'indirizzo di ritorno va messo nel Program Counter (\texttt{pc}) per concludere automaticamente la chiamata: \texttt{pop \{r4, r5, r6, pc\}}. \\
    \textbf{Perché scartare le altre:} L'opzione A manca del writeback (\texttt{!}), congelando il puntatore in un loop letale. Le opzioni C e D usano incrementi di 4 byte, errati poiché i dati sono a 64 bit (8 byte). Inoltre, pop nel \texttt{lr} (C) richiederebbe un'istruzione aggiuntiva per saltare indietro.

    \item \textbf{Domanda:} Viene dato un frammento C e una traduzione Intel x86-64 incompleta. Quale istruzione deve essere inserita al posto di \texttt{X1} per completare correttamente la traduzione?
\begin{verbatim}
int mistery (int a[], int n, int b[], int m) {
    int res = 1;
    for (int i=0; i<n && (res<a[i] || res<b[i]) && i<m; i++) {
        res *= a[i] + b[i/2];
    }
    return res;
}
\end{verbatim}
    \begin{enumerate}[label=(\Alph*)]
        \item \texttt{X1: addl \%edi, \%eax}
        \item \texttt{X1: imull \%edi, \%eax}
        \item \texttt{X1: addl \%edi, \%r8d}
        \item \texttt{X1: imull \%edi, \%r8d}
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (D)} \\
    \textbf{Perché è corretta:} Nel frammento assembly analizzato, la somma $a[i] + b[i/2]$ è stata appena calcolata nel registro \texttt{\%edi}. L'operazione richiesta per aggiornare \texttt{res} è una moltiplicazione (\texttt{*=}). La variabile di accumulo \texttt{res} risiede nel registro \texttt{\%r8d} (in quanto inizializzata a 1 all'esterno del ciclo e restituita in \texttt{\%eax} alla fine). L'istruzione corretta per moltiplicare i due e salvare il risultato in \texttt{\%r8d} è \texttt{imull \%edi, \%r8d}[cite: 68, 69, 70]. \\
    \textbf{Perché scartare le altre:} (A) e (C) effettuano una banale addizione ignorando l'operatore prodotto (\texttt{*=}) del codice C. L'opzione (B) accumulerebbe la moltiplicazione nel registro \texttt{\%eax} che in questo contesto interno è adoperato per altri scopi, distruggendo la variabile \texttt{res} che vive in \texttt{\%r8d}.

\newpage

    \item \textbf{Domanda:} Quale tipo di hazard si potrebbe verificare durante l'esecuzione di queste istruzioni in un processore con pipeline a 5 stadi (IF, ID, EX, MEM, WB)?
\begin{verbatim}
sub x4, x5, x6
add x7, x5, x8
add x9, x6, x10
...
\end{verbatim}
    \begin{enumerate}[label=(\Alph*)]
        \item Hazard di controllo.
        \item Hazard strutturale.
        \item Hazard di dati.
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (A)} \\
    \textbf{Perché è corretta:} L'ultima riga è un'istruzione di salto condizionato (\texttt{jpz x4, label}) il cui comportamento altererà il normale flusso di fetch delle istruzioni successive a seconda del valore del registro \texttt{x4}. Poiché la destinazione non è immediatamente predicibile, si forma nella pipeline un \textbf{Hazard di Controllo} che il processore dovrà gestire ritardando le operazioni finché l'indirizzo target non diventerà noto[cite: 75, 76, 77]. \\
    \textbf{Perché scartare le altre:} Non c'è alcun hazard strutturale, non essendoci collisione su risorse condivise. Benché \texttt{jpz} legga \texttt{x4} modificato dal primo \texttt{sub}, la distanza in pipeline (5 istruzioni intermedie) è ampiamente sufficiente a far completare la fase di WB per quel registro, per cui al momento dell'ingresso della \texttt{jpz} non si verifica un Hazard sui Dati.

    \item \textbf{Domanda:} Quale delle seguenti affermazioni descrive un vantaggio per l'uso di un bus asincrono rispetto a un bus sincrono?
    \begin{enumerate}[label=(\Alph*)]
        \item Un bus asincrono è molto più semplice da progettare rispetto a un bus sincrono.
        \item Un bus sincrono è più robusto al clock drift rispetto a un bus asincrono.
        \item Un bus asincrono opera anche tra dispositivi con frequenze e fasi di clock differenti.
        \item Un bus sincrono usa dei segnali di controllo più complessi rispetto a un bus asincrono.
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (C)} \\
    \textbf{Perché è corretta:} Il più grande pregio dei bus asincroni (e svantaggio di quelli sincroni) è il non fare affidamento su un unico clock globale, bensì su protocolli di *handshaking*. Questo consente a svariate periferiche eterogenee, con interfacce lente o con frequenze di clock differenti rispetto alla CPU, di comunicare fluidamente adattandosi alle diverse tempistiche. \\
    \textbf{Perché scartare le altre:} I bus asincroni richiedono protocolli a più "mani" e risultano fisicamente più complessi da progettare (A). I bus sincroni sono più rigidi, richiedono controlli semplici ma non gestiscono agilmente dispositivi multi-frequenza e sono limitati su lunghe distanze (B, D).

    \item \textbf{Domanda:} Sia una cache del tipo set associative con 8 vie, questa permette di archiviare 128 kB di dati su blocchi della dimensione di 64 bytes. Considerando che vogliamo accedere al dato con indirizzo \texttt{0x1FA58B} e che non lo troviamo nella cache, dopo averlo recuperato dalla memoria principale in quale blocco tra quelli elencati potremmo inserire questo dato?
    \begin{enumerate}[label=(\Alph*)]
        \item Nel blocco numero 256 della cache.
        \item Nel blocco numero 1023 della cache.
        \item Nel blocco numero 512 della cache.
        \item Nel blocco numero 1204 della cache.
        \item In nessuno dei blocchi elencati.
    \end{enumerate}
    \textbf{Risposta corretta: (D)} \\
    \textbf{Perché è corretta:} Strutturiamo la cache: per blocchi da 64 bytes servono $\log_2(64)=6$ bit di Offset. I blocchi totali sono $128\text{kB} / 64 = 2048$. Essendo a 8 vie, i Set sono $2048 / 8 = 256$ Set. Questo richiede $\log_2(256)=8$ bit di indice. Convertiamo l'indirizzo in binario: \texttt{0001 1111 1010 0101 1000 1011}. L'indice si trova nei bit da 6 a 13 e corrisponde a \texttt{10010110}, ovvero il Set \texttt{150} decimale. All'interno del Set 150 ci sono 8 vie fisiche contigue che vanno dal blocco numero $150 \times 8 = 1200$ al blocco $1200 + 7 = 1207$[cite: 93, 94]. Poiché il numero 1204 ricade in questo preciso range di slot fisici a disposizione per quel set, la risposta corretta è la D. \\
    \textbf{Perché scartare le altre:} Le opzioni A, B e C elencano numeri di blocco situati in zone di memoria della cache diametralmente lontane, assegnati a set differenti derivanti da probabili malinterpretazioni nel calcolo del modulo o delle basi.

    \item \textbf{Domanda:} Supponendo un processore che ha la facoltà di dividere l'esecuzione di una istruzione in più operazioni distinte, supponendo che la fetch dell'istruzione impieghi 300ps, la decode impieghi 200ps, l'esecuzione impieghi 400ps, l'accesso alla memoria impieghi 500ps e la scrittura del risultato impieghi 100ps. Quale è il possibile, teorico, fattore di miglioramento se strutturassimo il processore con una pipeline a 5 stadi (IF, ID, EX, MEM, WB)?
    \begin{enumerate}[label=(\Alph*)]
        \item 2.5
        \item 3.0
        \item 4.5
...
\newpage
    ...
    \textbf{Risposta corretta: (B)} \\
    \textbf{Perché è corretta:} Il tempo totale senza pipeline è dato dalla somma dei ritardi di ciascuno stadio: $300 + 200 + 400 + 500 + 100 = 1500$ ps. Quando si implementa la pipeline, la durata del singolo ciclo di clock è determinata unicamente dal collo di bottiglia, ovvero lo stadio più lento, che in questo caso impiega $500$ ps. Il potenziale fattore di miglioramento (Speedup) teorico è pari al tempo sequenziale diviso il nuovo tempo di ciclo: $1500 / 500 = 3.0$. \\
    \textbf{Perché scartare le altre:} (D) varrebbe 5.0 unicamente se la pipeline fosse perfetta e idealmente bilanciata in stadi tutti della medesima durata (cosa falsa qui). Le altre opzioni derivano da somme errate o rapporti inversi.

    \item \textbf{Domanda:} Viene dato un frammento C e una traduzione ARM incompleta. Quali sono le due istruzioni da inserire al posto dei puntini per completare correttamente la traduzione?
\begin{verbatim}
void vectops (short *a, short *b, short *c, short n) {
    short i = 0;
    while (i < n) {
...
\end{verbatim}
    \begin{enumerate}[label=(\Alph*)]
        \item \texttt{suble ip, ip, lr; strhgt ip, [ro]}
        \item \texttt{suble ip, lr, ip; strhle ip, [ro]}
        \item \texttt{suble ip, lr, ip; strhgt ip, [ro]}
        \item \texttt{suble ip, ip, lr; strhle ip, [ro]}
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (D)} \\
    \textbf{Perché è corretta:} L'analisi del codice assembly rivela che ARM ottimizza l'\texttt{if/else} apponendo suffissi di predizione. Se $b[i] > c[i]$, ARM ha già calcolato correttamente tramite \texttt{subgt} e \texttt{strhgt}. La riga da compilare è il ramo \texttt{else}, e va eseguita solo nel caso logico opposto, ovvero quando $b[i] \leq c[i]$ (Less or Equal). La sottrazione C per l'else richiede di prendere $c[i]$ (mappato in \texttt{ip}) sottrargli $b[i]$ (\texttt{lr}) e salvare nel registro \texttt{ip}: \texttt{suble ip, ip, lr}. Infine lo store \textit{condizionale} deposita il risultato nell'array dest: \texttt{strhle ip, [ro]}. \\
    \textbf{Perché scartare le altre:} Nelle risposte A e C si tenta un'illogica inversione del salvataggio unendo suffissi \texttt{le} a suffissi \texttt{gt} (che farebbero abortire l'esecuzione dell'istruzione se i flag di Less-Equal fossero accesi). La risposta B sottrae in ordine inverso \texttt{lr - ip} scambiando di fatto $b[i] - c[i]$, errato per l'else.

\newpage

    \item \textbf{Domanda:} Viene dato un frammento C e una traduzione Intel x86-64 incompleta. Quali sono le due istruzioni da inserire al posto di \texttt{X1} e \texttt{X2} per completare correttamente la traduzione?
\begin{verbatim}
int vectops (int a[], int n, int b, int c) {
    int i=0, prod=1;
    while (i++ < n) {
        if (a[i] > b)      a[i] = b;
        else if (a[i] > c) a[i] = c;
...
\end{verbatim}
    \begin{enumerate}[label=(\Alph*)]
        \item \texttt{X1: imull 4(\%r8), \%edi} | \texttt{X2: movl \%edx, 4(\%rax)}
        \item \texttt{X1: imull 4(\%r8), \%edi} | \texttt{X2: movl \%ecx, 4(\%rax)}
        \item \texttt{X1: addl 4(\%r8), \%edi} | \texttt{X2: movl \%edx, 4(\%rax)}
        \item \texttt{X1: addl 4(\%r8), \%edi} | \texttt{X2: movl \%ecx, 4(\%rax)}
        \item Nessuna delle precedenti.
    \end{enumerate}
    \textbf{Risposta corretta: (A)} \\
    \textbf{Perché è corretta:} Rilevando la semantica C e i blocchi del loop tradotti in Assembly x86, si deduce che manca la porzione del prodotto iterativo \texttt{prod *= a[i]} (eseguita sempre a fine blocco) e l'assegnamento del ramo if \texttt{a[i] = b}. Il prodotto logico (\texttt{prod}) è stoccato in \texttt{\%edi}. L'operazione corretta è la moltiplicazione CISC di un dato caricato da memoria con offset: \texttt{imull 4(\%r8), \%edi} in \texttt{X1}[cite: 140, 142]. Invece in \texttt{X2}, corrispondente alla gestione dell'assegnamento if (prima del \texttt{jmp L4}), dobbiamo spostare il valore scalare di \texttt{b} (in \texttt{\%edx}) nella cella \texttt{a[i]} (puntata da \texttt{rax} offset 4): \texttt{movl \%edx, 4(\%rax)}. \\
    \textbf{Perché scartare le altre:} Le opzioni (C) e (D) usano addizioni (\texttt{addl}) rimpiazzando l'operatore matematico del ciclo (il prodotto). L'opzione (B) utilizza in \texttt{X2} il registro \texttt{\%ecx}, assegnando di fatto \texttt{c} e non \texttt{b} quando viene rilevata la condizione \texttt{a[i] > b}.
...
\section{Preparazione all'Esame Orale: Domande Teoriche e Approfondimenti}

In questa sezione vengono affrontate in maniera discorsiva e dettagliata le principali domande poste frequentemente all'esame orale. Queste risposte costituiscono un'ottima base per ripassare i concetti e dimostrare padronanza tecnica.

\subsection{1. Spiegare la gerarchia della memoria}
Il problema storico dell'architettura dei calcolatori è conciliare la necessità di avere memorie \textbf{capienti}, \textbf{veloci} ed \textbf{economiche}. Fisicamente, una memoria non può soddisfare tutti e tre i requisiti: memorie veloci (come la SRAM) costano tantissimo e occupano molto spazio sul chip (bassa densità), mentre memorie capienti (come dischi o DRAM) sono lente ed economiche. 

Per risolvere il problema si utilizza una \textbf{gerarchia}. Si sfrutta il \textit{Principio di Località} per dare all'utente l'illusione di possedere una memoria grande quanto il livello più capiente (e lento) ma veloce quanto il livello più rapido (vicino al processore).
La gerarchia si compone tipicamente di:

\begin{enumerate}
    \item \textbf{Registri della CPU:} Costruiti in SRAM, i più veloci in assoluto ma limitati a poche decine o centinaia di byte.
    \item \textbf{Cache (L1, L2, L3):} Costruite in SRAM, vicinissime o integrate nel processore. Ordine di grandezza dei Kilobyte o Megabyte. Latenza di pochi nanosecondi.
    \item \textbf{Memoria Principale (RAM):} Costruita in DRAM (condensatore + transistor), molto densa ed economica. Ordine dei Gigabyte, ma latenze nell'ordine delle decine o centinaia di nanosecondi.
    \item \textbf{Memorie di Massa (Hard Disk, SSD):} Archiviazione permanente, non volatile, ordine dei Terabyte. Tempi di accesso nell'ordine dei millisecondi (milioni di volte più lenti della CPU).
\end{enumerate}

\subsection{2. Definire i principi di località spaziale e temporale}
Il successo dell'intera gerarchia di memoria, e in primis della cache, si fonda su due assunti statistici verificati sul comportamento reale dei programmi:
\begin{itemize}
    \item \textbf{Località Temporale:} Se un programma ha appena fatto accesso a una determinata cella di memoria, è altamente probabile che \textit{a brevissimo tempo} faccia di nuovo accesso a quella stessa cella. Un esempio tipico sono le variabili contatore all'interno di un ciclo \texttt{for}, che vengono lette e sovrascritte ad ogni iterazione.
    \item \textbf{Località Spaziale:} Se un programma fa accesso a una locazione di memoria, è altamente probabile che nei passi immediatamente successivi richieda l'accesso alle celle di memoria \textit{fisicamente adiacenti}. Un classico esempio è lo scorrimento di un array o l'esecuzione sequenziale delle istruzioni del programma in \texttt{.text}.
\end{itemize}
Grazie alla prima, manteniamo in cache i dati usati da poco. Grazie alla seconda, quando si verifica un \textit{miss}, non portiamo in cache la singola word richiesta, ma preleviamo un intero \textbf{blocco} (linea di cache) che comprende la word richiesta e quelle adiacenti.

\subsection{3. Le varie tipologie di cache e la lettura Set-Associativa}
Esistono tre architetture hardware per mappare la memoria nella cache:
\begin{enumerate}
    \item \textbf{Direct Mapped (A mappatura diretta):} Ogni blocco della RAM ha \textit{un solo} blocco specifico nella cache in cui può essere salvato (calcolato tramite l'operazione \texttt{Indirizzo MODULO NumeroBlocchiCache}). Se due indirizzi competono per lo stesso slot, si sfrattano a vicenda creando molti conflitti (\textit{conflict miss}).
    \item \textbf{Fully Associative (Completamente associativa):} Un blocco può essere posizionato in \textit{qualsiasi} riga libera della cache. Abbassa drasticamente i miss, ma per cercare il dato bisogna confrontare contemporaneamente in hardware tutti i \textit{Tag} della cache. Estremamente costosa, impraticabile per grandi dimensioni.
    \item \textbf{Set Associativa a N vie:} Un compromesso eccellente. La cache è divisa in "Insiemi" (\textit{Set}). Un blocco RAM è vincolato a finire in uno specifico Set (tramite operazione modulo), ma \textit{all'interno} di quel Set può essere collocato in una qualsiasi delle $N$ vie a disposizione.
\end{enumerate}

\textbf{Lettura da una Set-Associativa:}
Quando la CPU cerca un dato, frammenta l'indirizzo generato in tre parti: \textit{Tag}, \textit{Indice} e \textit{Offset}.
\begin{itemize}
    \item L'\textbf{Indice} viene usato per selezionare lo specifico \textit{Set} della cache.
    \item All'interno di quel Set vi sono $N$ blocchi. La circuiteria estrae \textit{tutti gli $N$ Tag} dei blocchi presenti e li confronta \textbf{in parallelo} (simultaneamente) con il \textit{Tag} derivato dall'indirizzo richiesto.
    \item Se c'è una coincidenza e il bit di validità ($V$) è 1, si ha un \textit{Hit}. L'uscita del comparatore logico piloterà un Multiplexer che selezionerà esattamente la via giusta, restituendo infine la specifica word calcolata tramite l'\textbf{Offset}.
\end{itemize}

\subsection{4. Il concetto di Pipeline: vantaggi, stadi ed Hazard}
La \textbf{Pipeline} è una tecnica architetturale che aumenta il \textbf{throughput} del processore, applicando il principio della catena di montaggio di Henry Ford. Invece di eseguire un'intera istruzione (es. 800ps) prima di prelevare la successiva, l'esecuzione viene sminuzzata in stadi bilanciati (solitamente 5, di pari durata, es. 200ps). In questo modo, ad ogni ciclo di clock (200ps), una nuova istruzione entra e un'istruzione terminata esce, permettendo alla CPU di lavorare su 5 istruzioni in parallelo in fasi diverse.

Gli \textbf{Stadi RISC classici} sono:
\textbf{IF} (Instruction Fetch), \textbf{ID} (Instruction Decode \& Register Read), \textbf{EX} (Execute / ALU), \textbf{MEM} (Memory Access), \textbf{WB} (Write-Back nel registro).

\newpage

I problemi di sovrapposizione si chiamano \textbf{Hazard}:
\begin{itemize}
    \item \textbf{Hazard Strutturali:} Due istruzioni tentano di usare fisicamente la stessa risorsa nello stesso ciclo (es. leggere un dato e prelevare un'istruzione usando l'unica memoria). Si risolve duplicando l'hardware (es. Cache Istruzioni e Cache Dati separate).
    \item \textbf{Hazard sui Dati (Data Hazard):} Un'istruzione in fase \textit{EX} ha bisogno del risultato di una \textit{add} che la precede ma che non è ancora arrivata alla fine per scriverlo (\textit{WB}). Si risolve con il \textbf{Forwarding} (la ALU pesca il risultato appena finito e lo ricicla in ingresso) o, nei peggiori casi (dopo una \textit{Load}), si stallerà la pipeline inserendo delle \textbf{bolle} (Nop) per aspettare che la memoria consegni il dato.
    \item \textbf{Hazard di Controllo (Branch Hazard):} Dopo una \textit{beq}, la CPU non sa subito da quale riga prelevare la successiva (non ha ancora calcolato l'uguaglianza e il target). Si risolve con la \textbf{Branch Prediction}: hardware specifico prova a indovinare statisticamente l'esito. Se sbaglia, svuota la pipeline sprecando cicli (\textit{Flush}).
\end{itemize}

\subsection{5. Differenza tra Interrupt e Traps}
Entrambe causano una transizione imprevista del flusso verso il Sistema Operativo (passando a modalità \textit{Supervisor}), ma le origini e le tempistiche sono opposte.
\begin{itemize}
    \item Le \textbf{Eccezioni Interne (Traps / Faults)} sono generate \textbf{dall'interno} del processore a causa dell'esecuzione di una singola e specifica istruzione problematica (es. divisione per zero, page fault, istruzione illegale o \texttt{ecall}). Sono eventi del tutto \textbf{sincroni}: avvengono esattamente su quella istruzione.
    \item Gli \textbf{Interrupts} sono interruzioni hardware generate \textbf{dall'esterno} del processore (es. una tastiera, la scheda di rete, un timer, il disco fisso). Sono totalmente scorrelate dall'istruzione che la CPU sta processando e sono eventi \textbf{asincroni}.
\end{itemize}

\subsection{6. Polling vs I/O Asincrono e Bus}
\textbf{L'I/O Polling (Attesa Attiva):} La CPU dialoga con le periferiche (molto lente) attraverso registri Memory Mapped. Nel \textit{polling}, la CPU entra in un ciclo infinito in cui legge ripetutamente un \textit{Registro di Stato} della periferica (\texttt{while (!ready);}), attendendo che l'hardware setti il bit di disponibilità per poi inviare il dato. Sebbene semplice da programmare, genera un enorme spreco di preziosi cicli di CPU (fino al 100\% di CPU Load inutilmente).
Per questo motivo si delega la comunicazione agli \textit{Interrupts} o al \textit{DMA} (Direct Memory Access).

\textbf{Bus sincroni vs asincroni:}
\begin{itemize}
    \item I \textbf{Bus Sincroni} operano comandati da un clock globale (tipicamente il bus Processore-Memoria). Sono velocissimi, ma impongono che ogni entità collegata rispetti una specifica tempistica: non si prestano a periferiche eterogenee di varia velocità.
    \item I \textbf{Bus Asincroni} non hanno un clock condiviso (es. bus di I/O o di sistema estesi). La sincronizzazione avviene con un protocollo di \textbf{Handshaking} a quattro mani (Richiesta, Conferma di pronto, Ricevuta) adattando dinamicamente le tempistiche della comunicazione alle capacità della singola periferica.
\end{itemize}

\newpage

\subsection{7. Spiegare lo schema del processore (RISC-V) e la \texttt{beq}}
Lo schema del Datapath di base (a singolo ciclo) unisce gli stadi logici visti per la pipeline in un unico grande circuito combinatorio che viene alimentato e termina in un singolo colpo di clock.
Tutto inizia dal \textbf{PC (Program Counter)}, che indica quale istruzione prelevare dalla \textbf{Memoria Istruzioni}. Un sommatore isolato ne ricalcola subito il +4. 
L'istruzione generata spinge l'\textbf{Unità di Controllo} ad asserire segnali (0/1) per pilotare i vari MUX (Multiplexer) del datapath, scegliendo cosa deve scorrere nei fili.

\textbf{I Segnali di Controllo per la Branch (\texttt{beq}):}
Per la \texttt{beq x5, x6, offset}, la CPU deve confrontare \texttt{x5} e \texttt{x6} e saltare se sono uguali.
L'Unità di Controllo leggerà l'Opcode (codice operativo) e configurerà la CPU così:
\begin{itemize}
    \item \texttt{\textbf{Branch = 1}}: Attiva il potenziale Multiplexer di ricalcolo del PC che si trova alla fine del datapath.
    \item \texttt{\textbf{ALUOp = 01}}: Ordina al controllore secondario della ALU di eseguire una \textbf{sottrazione}.
    \item \texttt{\textbf{ALUSrc = 0}}: Indica che il secondo operando che entra in ALU \textit{non} deve essere l'offset, ma deve essere letto dal banco dei registri (\texttt{x6}).
    \item \texttt{\textbf{RegWrite = 0} / \textbf{MemWrite = 0}}: La \textit{branch} non modifica né registri generali né la memoria dati. Quei circuiti vanno tenuti spenti per sicurezza.
\end{itemize}
Contemporaneamente, l'addizionatore in alto calcola l'indirizzo di offset (esteso in segno e \textit{shiftato} per 2). 
Infine: se la ALU ha calcolato $x5 - x6 = 0$, la sua uscita hardware speciale \textbf{Zero} diventa $1$. Essendo una porta logica AND attivata su \texttt{Branch} e \texttt{Zero}, solo in questo scenario il Mux finale devierà il prossimo PC sull'indirizzo appena calcolato, "prendendo" di fatto il salto.



\subsection{8. Istruzione \texttt{lea} in Assembly Intel x86-64}
L'istruzione \textbf{Load Effective Address} (\texttt{lea}) sfrutta una particolarità CISC di Intel. Benché abbia la sintassi di un'operazione verso la RAM (ad es. \texttt{leaq (\%rsi, \%rdi, 4), \%rax}), \textit{non tocca mai la memoria}.
Passa invece gli operandi all'\textit{Address Generation Unit} hardware della CPU, facendogli eseguire un'operazione aritmetica complessa (in questo caso $rsi + rdi \times 4$) e riversa puramente il risultato aritmetico calcolato all'interno del registro di destinazione (\texttt{\%rax}). A differenza dell'istruzione \texttt{add}, \texttt{lea} non "sporca" nessuno dei due registri in ingresso ed è usatissima dal compilatore per operazioni aritmetiche ottimizzate o incrementi.

\newpage

\subsection{9. Toolchain: fasi ed esecuzione}
Il processo di trasformazione dal linguaggio C all'eseguibile passa tramite 4 fasi della "catena degli strumenti":
\begin{enumerate}
    \item \textbf{Preprocessore:} Risolve direttive testuali (\texttt{\#include}, \texttt{\#define}), incollando e sostituendo testo puro.
    \item \textbf{Compilatore:} Analizza la logica di alto livello e traduce in linguaggio macchina mnemonico (\textbf{Assembly}, file \texttt{.s}). 
    \item \textbf{Assemblatore (Assembler):} Sostituisce le istruzioni pseudo (\texttt{mv, li, j}) in istruzioni vere. Codifica le istruzioni mnemonico in vero \textbf{Linguaggio Macchina binario}, producendo i blocchi \texttt{.data} e \texttt{.text}. Restituisce un File Oggetto non eseguibile (file \texttt{.o}), contenente "buchi" non risolti (Tabella di Rilocazione) e le interfacce per comunicare all'esterno (Symbol Table).
    \item \textbf{Linker:} Assembla e amalgama più file oggetto insieme alle librerie (Statiche o Dinamiche). Assegna a tutti uno spazio di memoria assoluto (sostituendo gli indirizzi relativi) ed esegue il \textit{patching} (Rilocazione) rimpiazzando i "buchi" con i veri indirizzi fisici prima di sfornare l'eseguibile (\texttt{.exe} o \texttt{a.out}).
\end{enumerate}

\subsection{10. Write-back in Assembly ARM (e implementazione in RISC-V)}
Una delle peculiarità dell'istruzione di Load e Store in ARM è l'utilizzo dell'operatore \textbf{Write-Back} (espresso col suffisso del punto esclamativo \texttt{!}, o tramite post-indexing).
Un'istruzione come \texttt{ldr r3, [r0, \#4]!} estrae il valore dalla memoria all'indirizzo $r0+4$, e in un singolo colpo di clock aggiorna \textit{anche} il registro di base \texttt{r0}, che ora conterrà proprio il valore \texttt{r0+4}.

\textbf{Domanda Bonus: Come implementereste l'equivalente ARM in RISC-V?}
L'architettura RISC-V si rifiuta di fare troppe cose nello stesso colpo di clock (per mantenere stadi rapidi). RISC-V permette un'unica operazione di Write-Back alla volta per istruzione (scrivendo, in questo caso, sul registro \texttt{r3}). Poiché in ARM stiamo aggiornando ben *due registri in contemporanea* (la destinazione \texttt{r3} col dato RAM e la base \texttt{r0} col nuovo offset calcolato dalla ALU), un'implementazione RISC-V esatta richiederebbe \textbf{due istruzioni sequenziali distinte}:
\begin{verbatim}
ld   x3, 4(x0)        // Carico il dato in x3 (Equivalente al LDR semplice)
addi x0, x0, 4        // Incremento separatamente il registro base x0
\end{verbatim}
Per averlo nativo, la microarchitettura RISC-V dovrebbe essere pesantemente modificata, inserendo due reti di feedback distinte per il \textit{Write-Back stage} e costringendo il banco dei registri ad accettare \textit{due scritture} simultanee per ciclo, complicando il datapath a dismisura e rendendolo più costoso.
